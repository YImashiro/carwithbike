...
class ServoMotor:
    ...
    def steeringControl(self,percentage):
        ...
        if percentage > self.percentage:
            for i in range(self.percentage,percentage):
                self.pwm.ChangeDutyCycle(self.straight + self.__steeringControl(i))
                time.sleep(0.01)
        elif percentage < self.percentage:
            for i in range(self.percentage,percentage,-1):
                self.pwm.ChangeDutyCycle(self.straight + self.__steeringControl(i))
                time.sleep(0.01)
